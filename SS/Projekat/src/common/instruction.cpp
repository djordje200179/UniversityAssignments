...
instruction instruction::make_call(bool mem_indirect,
								   uint8_t reg1,
								   uint8_t reg2,
								   uint16_t displacement) {
	return instruction{.code = 0x2,
					   .mode = mem_indirect,
					   .reg1 = reg1,
					   .reg2 = reg2,
...
instruction instruction::make_jump(jump_mode mode,
								   bool mem_indirect,
								   uint8_t reg1,
								   uint8_t reg2,
								   uint8_t reg3,
								   uint16_t displacement) {
	return instruction{.code = 0x3,
...
instruction instruction::make_xchg(uint8_t reg1, uint8_t reg2) {
	return instruction{.code = 0x4, .reg2 = reg1, .reg3 = reg2};
}
...
instruction instruction::make_shift(bool right,
									...
									uint8_t reg3) {
	return instruction{
		.code = 0x7, .mode = right, .reg1 = reg1, .reg2 = reg2, .reg3 = reg3};
}
