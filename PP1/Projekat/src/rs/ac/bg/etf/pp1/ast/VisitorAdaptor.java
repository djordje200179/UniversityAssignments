// generated with ast extension for cup
// version 0.8
// 13/0/2024 13:23:46


package rs.ac.bg.etf.pp1.ast;

public abstract class VisitorAdaptor implements Visitor { 

    public void visit(Mulop Mulop) { }
    public void visit(VarRef VarRef) { }
    public void visit(Relop Relop) { }
    public void visit(NspDecls NspDecls) { }
    public void visit(ElemAccess ElemAccess) { }
    public void visit(VarQuantity VarQuantity) { }
    public void visit(DesStmtList DesStmtList) { }
    public void visit(ActParams ActParams) { }
    public void visit(Addop Addop) { }
    public void visit(ConditionTry ConditionTry) { }
    public void visit(GlobalDecls GlobalDecls) { }
    public void visit(Factor Factor) { }
    public void visit(CondTerm CondTerm) { }
    public void visit(MethodSign MethodSign) { }
    public void visit(Term Term) { }
    public void visit(Condition Condition) { }
    public void visit(FormParams FormParams) { }
    public void visit(ForDesStmtList ForDesStmtList) { }
    public void visit(StmtList StmtList) { }
    public void visit(Expr Expr) { }
    public void visit(ActPars ActPars) { }
    public void visit(DesOpt DesOpt) { }
    public void visit(DesignatorStatement DesignatorStatement) { }
    public void visit(Const Const) { }
    public void visit(VarDecls VarDecls) { }
    public void visit(Statement Statement) { }
    public void visit(VarDecl VarDecl) { }
    public void visit(ConstDecls ConstDecls) { }
    public void visit(CondFact CondFact) { }
    public void visit(VarTypedDecls VarTypedDecls) { }
    public void visit(DesUnpackList DesUnpackList) { }
    public void visit(ForCond ForCond) { }
    public void visit(GlobalDecl GlobalDecl) { }
    public void visit(FormPars FormPars) { }
    public void visit(FormParam FormParam) { }
    public void visit(MethodDecls MethodDecls) { }
    public void visit(CondOrJoiner CondOrJoiner) { visit(); }
    public void visit(CondTermCondition CondTermCondition) { visit(); }
    public void visit(ConditionList ConditionList) { visit(); }
    public void visit(ConditionTryDerived1 ConditionTryDerived1) { visit(); }
    public void visit(ConditionOk ConditionOk) { visit(); }
    public void visit(CondFactTerm CondFactTerm) { visit(); }
    public void visit(CondTermList CondTermList) { visit(); }
    public void visit(CondFactExpr CondFactExpr) { visit(); }
    public void visit(Comparison Comparison) { visit(); }
    public void visit(SingularAccess SingularAccess) { visit(); }
    public void visit(ArrayElemAccess ArrayElemAccess) { visit(); }
    public void visit(Designator Designator) { visit(); }
    public void visit(NspVarRef NspVarRef) { visit(); }
    public void visit(BlankVarRef BlankVarRef) { visit(); }
    public void visit(ExprNegTerm ExprNegTerm) { visit(); }
    public void visit(ExprTerm ExprTerm) { visit(); }
    public void visit(ExprTermList ExprTermList) { visit(); }
    public void visit(FactorTerm FactorTerm) { visit(); }
    public void visit(FactorTermList FactorTermList) { visit(); }
    public void visit(DesignatorFactor DesignatorFactor) { visit(); }
    public void visit(CallFactor CallFactor) { visit(); }
    public void visit(NewArrayFactor NewArrayFactor) { visit(); }
    public void visit(GroupedFactor GroupedFactor) { visit(); }
    public void visit(ConstFactor ConstFactor) { visit(); }
    public void visit(ActParam ActParam) { visit(); }
    public void visit(ActParamsDerived2 ActParamsDerived2) { visit(); }
    public void visit(ActParamsDerived1 ActParamsDerived1) { visit(); }
    public void visit(ActParsDerived2 ActParsDerived2) { visit(); }
    public void visit(ActParsDerived1 ActParsDerived1) { visit(); }
    public void visit(CallBefore CallBefore) { visit(); }
    public void visit(Call Call) { visit(); }
    public void visit(LessEqualRelop LessEqualRelop) { visit(); }
    public void visit(LessRelop LessRelop) { visit(); }
    public void visit(GreaterEqualRelop GreaterEqualRelop) { visit(); }
    public void visit(GreaterRelop GreaterRelop) { visit(); }
    public void visit(InequalityRelop InequalityRelop) { visit(); }
    public void visit(EqualityRelop EqualityRelop) { visit(); }
    public void visit(PercentMulop PercentMulop) { visit(); }
    public void visit(SlashMulop SlashMulop) { visit(); }
    public void visit(AsteriskMulop AsteriskMulop) { visit(); }
    public void visit(MinusAddOp MinusAddOp) { visit(); }
    public void visit(PlusAddOp PlusAddOp) { visit(); }
    public void visit(DesOptEmpty DesOptEmpty) { visit(); }
    public void visit(DesOptDes DesOptDes) { visit(); }
    public void visit(DesUnpackListDerived2 DesUnpackListDerived2) { visit(); }
    public void visit(DesUnpackListDerived1 DesUnpackListDerived1) { visit(); }
    public void visit(DesUnpackStmt DesUnpackStmt) { visit(); }
    public void visit(DesDecStmt DesDecStmt) { visit(); }
    public void visit(DesIncStmt DesIncStmt) { visit(); }
    public void visit(DesignatorStatementDerived1 DesignatorStatementDerived1) { visit(); }
    public void visit(DesAssignStmt DesAssignStmt) { visit(); }
    public void visit(DesCallStmt DesCallStmt) { visit(); }
    public void visit(DesStmtListDerived2 DesStmtListDerived2) { visit(); }
    public void visit(DesStmtListDerived1 DesStmtListDerived1) { visit(); }
    public void visit(ForDesStmtListDerived2 ForDesStmtListDerived2) { visit(); }
    public void visit(ForDesStmtListDerived1 ForDesStmtListDerived1) { visit(); }
    public void visit(InfiniteFor InfiniteFor) { visit(); }
    public void visit(FiniteFor FiniteFor) { visit(); }
    public void visit(ForAfterCond ForAfterCond) { visit(); }
    public void visit(ForBeforeCond ForBeforeCond) { visit(); }
    public void visit(ForSign ForSign) { visit(); }
    public void visit(IfElseStart IfElseStart) { visit(); }
    public void visit(IfAfterCond IfAfterCond) { visit(); }
    public void visit(IfBeforeCond IfBeforeCond) { visit(); }
    public void visit(StatementDerived1 StatementDerived1) { visit(); }
    public void visit(ForStmt ForStmt) { visit(); }
    public void visit(IfStmt IfStmt) { visit(); }
    public void visit(IfElseStmt IfElseStmt) { visit(); }
    public void visit(DesStmt DesStmt) { visit(); }
    public void visit(PrintStmt PrintStmt) { visit(); }
    public void visit(PrintWidthStmt PrintWidthStmt) { visit(); }
    public void visit(ReadStmt ReadStmt) { visit(); }
    public void visit(ContinueStmt ContinueStmt) { visit(); }
    public void visit(BreakStmt BreakStmt) { visit(); }
    public void visit(VoidReturnStmt VoidReturnStmt) { visit(); }
    public void visit(ValueReturnStmt ValueReturnStmt) { visit(); }
    public void visit(StmtListDerived2 StmtListDerived2) { visit(); }
    public void visit(StmtListDerived1 StmtListDerived1) { visit(); }
    public void visit(FormParamDerived1 FormParamDerived1) { visit(); }
    public void visit(FormParamOk FormParamOk) { visit(); }
    public void visit(FormParamsDerived2 FormParamsDerived2) { visit(); }
    public void visit(FormParamsDerived1 FormParamsDerived1) { visit(); }
    public void visit(FormParsDerived2 FormParsDerived2) { visit(); }
    public void visit(FormParsDerived1 FormParsDerived1) { visit(); }
    public void visit(VoidMethodSign VoidMethodSign) { visit(); }
    public void visit(ValueMethodSign ValueMethodSign) { visit(); }
    public void visit(MethodDecl MethodDecl) { visit(); }
    public void visit(MethodDeclsDerived2 MethodDeclsDerived2) { visit(); }
    public void visit(MethodDeclsDerived1 MethodDeclsDerived1) { visit(); }
    public void visit(SingularVar SingularVar) { visit(); }
    public void visit(ArrayVar ArrayVar) { visit(); }
    public void visit(VarDeclDerived1 VarDeclDerived1) { visit(); }
    public void visit(VarDeclOk VarDeclOk) { visit(); }
    public void visit(VarDeclsDerived2 VarDeclsDerived2) { visit(); }
    public void visit(VarDeclsDerived1 VarDeclsDerived1) { visit(); }
    public void visit(VarTypedDecl VarTypedDecl) { visit(); }
    public void visit(VarTypedDeclsDerived2 VarTypedDeclsDerived2) { visit(); }
    public void visit(VarTypedDeclsDerived1 VarTypedDeclsDerived1) { visit(); }
    public void visit(BoolConst BoolConst) { visit(); }
    public void visit(CharConst CharConst) { visit(); }
    public void visit(IntConst IntConst) { visit(); }
    public void visit(ConstDecl ConstDecl) { visit(); }
    public void visit(ConstDeclsDerived2 ConstDeclsDerived2) { visit(); }
    public void visit(ConstDeclsDerived1 ConstDeclsDerived1) { visit(); }
    public void visit(ConstTypedDecl ConstTypedDecl) { visit(); }
    public void visit(GlobalVarDecl GlobalVarDecl) { visit(); }
    public void visit(GlobalConstDecl GlobalConstDecl) { visit(); }
    public void visit(GlobalDeclsDerived2 GlobalDeclsDerived2) { visit(); }
    public void visit(GlobalDeclsDerived1 GlobalDeclsDerived1) { visit(); }
    public void visit(Type Type) { visit(); }
    public void visit(NspName NspName) { visit(); }
    public void visit(NspDecl NspDecl) { visit(); }
    public void visit(NspDeclsDerived2 NspDeclsDerived2) { visit(); }
    public void visit(NspDeclsDerived1 NspDeclsDerived1) { visit(); }
    public void visit(ProgName ProgName) { visit(); }
    public void visit(Program Program) { visit(); }


    public void visit() { }
}
